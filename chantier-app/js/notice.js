...
const NOTICE_TEXT = {
  fr: {
    ...
    sections: [
      ...
      ['Documents de conformité', "Pour respecter les obligations légales (détachement, sécurité électrique) et les exigences des clients : déclaration Limosa-1, certificat A1, VCA et habilitation électrique BA4/BA5 (date de validité et copie du document) ; carte d'identité et aptitude médicale : <strong>date de validité uniquement</strong>, aucune copie ni donnée médicale. Ces documents ne sont visibles que par vous et l'administrateur ; le client ne reçoit qu'un résumé « conforme / à renouveler ». Base légale : obligation légale et intérêt légitime (RGPD art. 6.1.c et f). Vous confirmez aussi votre présence la veille de chaque jour de chantier."],
      ['Combien de temps ?', "Position GPS et photos de pointage : 12 mois. Heures et kilomètres : 5 ans. Rapports de chantier : durée du projet + 5 ans. Compte et documents de conformité : durée de la collaboration, puis suppression après 12 mois. Factures : 10 ans (obligation comptable)."],
  ...
  en: {
    ...
    sections: [
      ...
      ['Compliance documents', "To meet legal obligations (posting of workers, electrical safety) and client requirements: Limosa-1 declaration, A1 certificate, VCA and electrical authorisation BA4/BA5 (expiry date and a copy of the document); ID document and medical fitness: <strong>expiry date only</strong>, no copy and no medical data. Only you and the administrator can see these documents; the client only receives a 'compliant / to renew' summary. Legal basis: legal obligation and legitimate interest (GDPR Art. 6(1)(c) and (f)). You also confirm your attendance the day before each site day."],
      ['How long?', 'GPS position and clock-in photos: 12 months. Hours and mileage: 5 years. Site reports: project duration + 5 years. Account and compliance documents: duration of the collaboration, then deleted after 12 months. Invoices: 10 years (accounting obligation).'],
  ...
  ro: {
    ...
    sections: [
      ...
      ['Documente de conformitate', "Pentru a respecta obligațiile legale (detașare, siguranță electrică) și cerințele clienților: declarația Limosa-1, certificatul A1, VCA și autorizarea electrică BA4/BA5 (data de valabilitate și o copie a documentului); act de identitate și aviz medical: <strong>doar data de valabilitate</strong>, nicio copie și nicio dată medicală. Doar dvs. și administratorul vedeți aceste documente; clientul primește doar un rezumat „conform / de reînnoit”. Temei legal: obligație legală și interes legitim (RGPD art. 6 alin. (1) lit. c și f). De asemenea, confirmați prezența cu o zi înainte de fiecare zi de șantier."],
      ['Cât timp?', 'Poziția GPS și fotografiile de pontaj: 12 luni. Ore și kilometri: 5 ani. Rapoarte de șantier: durata proiectului + 5 ani. Contul și documentele de conformitate: durata colaborării, apoi ștergere după 12 luni. Facturi: 10 ani (obligație contabilă).'],
  ...
  zh: {
    ...
    sections: [
      ...
      ['合规证件', "为遵守法律义务（派遣、电气安全）及客户要求：Limosa-1 申报、A1 证明、VCA 及 BA4/BA5 电工资质（有效期及证件副本）；身份证件和体检合格：<strong>仅记录有效期</strong>，不保存副本，也不保存任何医疗数据。只有您本人和管理员能看到这些证件；客户只会收到“合规 / 需更新”的摘要。法律依据：法律义务及正当利益（GDPR 第 6(1)(c) 和 (f) 条）。您还需在每个工地日的前一天确认到场。"],
      ['保存多久？', 'GPS 位置和打卡照片：12 个月。工时和公里数：5 年。工地报告：项目期间 + 5 年。账户及合规证件：合作期间，结束后 12 个月删除。发票：10 年（会计义务）。'],
